...
OptionValue::OptionValue(double value)
{
    ...
    value_[index_zero] = boost::lexical_cast<string>(value);
}
...
bool ConfigOption::Init(cJSON* joption)
{
    ...
    while (jnode != NULL)
    {
        ...
        if (jcurr->type == cJSON_String)
        {
            ...
            {
                boost::shared_ptr<OptionValue> value(new OptionValue(jcurr->valuestring));
                option_[jcurr->string] = value;
            }
        }
        else if (jcurr->type == cJSON_Number)
        {
            boost::shared_ptr<OptionValue> value(new OptionValue(jcurr->valuedouble));
            option_[jcurr->string] = value;
        }
        else if (jcurr->type == cJSON_True || jcurr->type == cJSON_False)
        {
            boost::shared_ptr<OptionValue> value(new OptionValue((double)jcurr->valueint));
            option_[jcurr->string] = value;
        }
...
const cJSON* Config::JsonObject(const string& path) const
{
    ...
    for (size_t i = 1; i < vs.size(); ++i)
    {
        ...
        while (jnode != NULL)
        {
            if (jnode->string != NULL &&
                strlen(jnode->string) != 0 &&
