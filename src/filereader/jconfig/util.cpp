...
static int check_host(string &host) {
    ...
    static string top_domain_ending[] = 
        {".com", ".net", ".org", ".edu", ".gov", ".biz", ".info", ".name", ".mobi", ".mil", ".me",
        ".ad", ".ae", ".af", ".ag", ".ai", ".al", ".am", ".an", ".ao", ".aq",
        ".ar", ".as", ".at", ".au", ".aw", ".az", ".ba", ".bb", ".bd", ".be",
        ".bf", ".bg", ".bh", ".bi", ".bj", ".bm", ".bn", ".bo", ".br", ".bs",
        ".bt", ".bv", ".bw", ".by", ".bz", ".ca", ".cc", ".cf", ".cg", ".ch",
        ".ci", ".ck", ".cl", ".cm", ".cn", ".co", ".cq", ".cr", ".cu", ".cv",
        ".cx", ".cy", ".cz", ".de", ".dj", ".dk", ".dm", ".do", ".dz", ".ec",
        ".ee", ".eg", ".eh", ".es", ".et", ".ev", ".fi", ".fj", ".fk", ".fm",
        ".fo", ".fr", ".ga", ".gb", ".gd", ".ge", ".gf", ".gh", ".gi", ".gl",
        ".gm", ".gn", ".gp", ".gr", ".gt", ".gu", ".gw", ".gy", ".hk", ".hm",
        ".hn", ".hr", ".ht", ".hu", ".id", ".ie", ".il", ".in", ".io", ".iq",
        ".ir", ".is", ".it", ".jm", ".jo", ".jp", ".ke", ".kg", ".kh", ".ki",
        ".km", ".kn", ".kp", ".kr", ".kw", ".ky", ".kz", ".la", ".lb", ".lc",
        ".li", ".lk", ".lr", ".ls", ".lt", ".lu", ".lv", ".ly", ".ma", ".mc",
        ".md", ".mg", ".mh", ".ml", ".mm", ".mn", ".mo", ".mp", ".mq",
        ".ms", ".mt", ".mv", ".mw", ".mx", ".my", ".mz", ".na", ".nc", ".ne",
        ".nf", ".ng", ".ni", ".nl", ".no", ".np", ".nr", ".nt", ".nu", ".nz",
        ".om", ".qa", ".pa", ".pe", ".pf", ".pg", ".ph", ".pk", ".pl", ".pm",
        ".pn", ".pr", ".pt", ".pw", ".py", ".re", ".ro", ".ru", ".rw", ".sa",
        ".sb", ".sc", ".sd", ".se", ".sg", ".sh", ".si", ".sj", ".sk", ".sl",
        ".sm", ".sn", ".so", ".sr", ".st", ".su", ".sy", ".sz", ".tc", ".td",
        ".tf", ".tg", ".th", ".tj", ".tk", ".tm", ".tn", ".to", ".tp", ".tr",
        ".tt", ".tv", ".tw", ".tz", ".ua", ".ug", ".uk", ".us", ".uy", ".va",
        ".vc", ".ve", ".vg", ".vn", ".vu", ".wf", ".ws", ".ye", ".yu", ".za", 
        ".zm", ".zr", ".zw"};
    ...
    boost::replace_all(host, "，", ".");
    boost::replace_all(host, "；", ".");
    // 兼容一些异常情况，包括可以出现一些空格，可以出现两个.. 以及最末尾可以多余一个.
    boost::replace_all(host, "..", ".");
    if (host.length() < 3)
      return 0;
    ...
    for (size_t i = 0; i < host.length(); ++i) {
        if (host[i] != '.' && host[i] != '-' && host[i] != '_' && (!isdigit(host[i])) && (!isalpha(host[i])))
            return 0;
    }


    for (size_t i = 0; i < sizeof(top_domain_ending) / sizeof(top_domain_ending[0]); ++i) {
        size_t pos = host.rfind(top_domain_ending[i]);
        if (pos != string::npos && pos > 0 && pos + top_domain_ending[i].size() == host.size()) {
            int doc_pos = 0;
            if (host.at(0) == '.')
                doc_pos = 1;
...
int is_url(string &str) {
    // 对于host部分需要大小写转化，path部分，不需要进行大小写转换
    string tmp_str1 = str;
    ...
    if (protocol_pos != string::npos) {
        ...
        tmp_str = tmp_str.substr(protocol_pos + 3, tmp_str.length() - protocol_pos - 3);
    ...
    if (slash_pos != string::npos) {
        ...
        if (question_pos != string::npos) {
            str = str.substr(0, question_pos) + "/" + str.substr(question_pos);
        } else {
            str += "/";
        }
    }
...
double calc_square_sum(char *p, int n, short *freq) {
    double r = 0.0;
    if (freq[256]) memset(freq, 0, sizeof(short) * 257);
    freq[256] = 1;
    for (int i = 0; i < n; i++) {
        freq[(unsigned char)(p[i])]++;
    }
    //freq['<'] = freq['>'] = freq['b'] = freq['/'] = freq['\\'] = 0;
    for (int i = 0; i < 256; i++) {
        r += freq[i] * freq[i];
    }
    //printf("p=[%.*s] r=%lf\n", n, p, sqrt(r));
...
double calc_cosine(double s1, short *freq1, double s2, short *freq2) {
    double r = 0.0;
#ifdef __DEBUG
    if (!freq1[256] || !freq2[256]) {
        printf("[error] freq not init\n");
    }
#endif
    for (int i = 0; i < 256; i++) {
        r += freq1[i] * freq2[i];
    }
    //printf("s1=%lf,s2=%lf,cos=%lf\n", s1, s2, r / (s1*s2));
    return r / (s1 * s2);
}
...
int main() {
    ...
    "http;//www.wo.com",
    "http；//WWW.WO.COM",
    "HTTp://e.weibo.cOM/2705393843/zfvTU8nt2"
    };
    string a[20];
    //cout << b[0]; cout << " " << is_url(b[0]) << " " << b[0] << endl;
    //return 1;
    a[0] = "http://com.bj:8000000?pvareaid=100519";
    a[1] = "http://.bj:8000000/?pvareaid=100519";
    a[2] = "www,so,cm:80";
    a[3] = "www，so,cm:80";
    a[4] = "www。so,cm:80";
    a[5] = "@163.com";
    a[6] = "搜索.com";
    a[7] = "http://192.255.168.118";
    a[8] = "absdf.asdf.ss";
    for (int i = 0; i < 9; ++i) {
        cout << a[i]; cout << " " << is_url(a[i]) << " " << a[i] << endl;
    }
    for (int i = 0; i < 17; ++i) {
        cout << b[i]; cout << " " << is_url(b[i]) << " " << b[i] << endl;
    }
}
