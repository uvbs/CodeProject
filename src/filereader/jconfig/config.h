#ifndef __CLOUD_SEARCH_CONFIG_H__
#define __CLOUD_SEARCH_CONFIG_H__
...
class ConfigOption
{
    public:
        ...
        void PrintToString(std::string* output, const::std::string& indent = "") const;
        ...
        std::string GetStr(const std::string& key, TIndex index, const std::string& default_value = "") const
        {
            std::string str_index = boost::str(boost::format("%1%") % index);
            std::map<std::string, boost::shared_ptr<OptionValue> >::const_iterator iter = option_.find(key);
            if (iter == option_.end())
        ...
        template<typename TIndex>
        double GetNum(const std::string& key, TIndex index, double default_value = 0) const
        {
...
class Config
{
    public:
        ...
        // 淘汰接口
        boost::shared_ptr<ConfigOption> section(const std::string& section_name, uint32_t index = 0) const;
