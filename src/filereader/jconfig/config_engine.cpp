...
{
    if (jnode == NULL || jnode->type != cJSON_Array)
        return false;
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        if (jname == NULL || jname->type != cJSON_String || jname->valuestring == NULL || strlen(jname->valuestring) == 0)
            continue;

        jvalue = cJSON_GetObjectItem(jitem, "value");
        if (jvalue == NULL || jvalue->type != cJSON_String || jvalue->valuestring == NULL)
            continue;

        jkeep = cJSON_GetObjectItem(jitem, "keep");
        if (jkeep != NULL && jkeep->type == cJSON_String && jkeep->valuestring != NULL && strlen(jkeep->valuestring) != 0)
            str_keep = jkeep->valuestring;

        jencode = cJSON_GetObjectItem(jitem, "encode");
        if (jencode != NULL && (jencode->type == cJSON_False || jencode->type == cJSON_True))
            encode = jencode->valueint;


        jalias = cJSON_GetObjectItem(jitem, "alias");
        if (jalias != NULL && jalias->type == cJSON_String && jalias->valuestring != NULL && strlen(jalias->valuestring) != 0)
...
bool ConfigEngine::Init(const string& name, boost::shared_ptr<cloud_search::Config> config)
{
...
bool ConfigEngine::SendAllow(const string& hash, const string& ip) const
{
    ...
    //LOG(DEBUG) << "ConfigEngine::SendAllow DISALLOW " << name_ << " " << hash << " " << ip << " flow_rate:" << flow_rate_ << " modulus:" << modulus;
...
bool ConfigEngines::Init(boost::shared_ptr<cloud_search::Config> config)
{
