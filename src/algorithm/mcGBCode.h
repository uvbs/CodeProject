...
** @brief: ±àÂë×ª»»
***************************************************/
#ifndef __MAPTRIXC_GB18030_LIB__
#define __MAPTRIXC_GB18030_LIB__
...
template <class _T>
class mcGBCodeT : public _T
{
public:
	static void GB18030ToUnicode(WORD *Dst, int &nDst, BYTE *Src, int nSrc);
	static void GB18030ToUnicode(WORD *Dst, int &nDst, BYTE *Src);

	static void UnicodeToGB18030(BYTE *Dst, int &nDst, WORD *Src, int nSrc);
	static void UnicodeToGB18030(BYTE *Dst, int &nDst, WORD *Src);

	static void UnicodeToUTF8(BYTE *Dst, int &nDst, WORD *Src, int nSrc);
	static void UnicodeToUTF8(BYTE *Dst, int &nDst, WORD *Src);

	static void UTF8ToUnicode(WORD *Dst, int &nDst, BYTE *Src, int nSrc);
	static void UTF8ToUnicode(WORD *Dst, int &nDst, BYTE *Src);

protected:
	static WORD GB18030ToUnicode(BYTE *Src, int nLen);
	static void UnicodeToGB18030(BYTE *Dst, WORD u);
};

typedef mcGBCodeT<mcMTL> mcGBCode;
...
#define Is1stByte(c)	(0x81 <= (c) && (c) <= 0xFE)
#define Is2ndByte(c)	(0x40 <= (c) && (c) <= 0xFE && (c) != 0x7F)
#define Or2ndByte(c)	(0x30 <= (c) && (c) <= 0x39)
#define Is3rdByte(c)	(0x81 <= (c) && (c) <= 0xFE)
#define Is4thByte(c)	(0x30 <= (c) && (c) <= 0x39)

#define IsUDA1(a, b)	(0xAA <= (a) && (a) <= 0xAF && 0xA1 <= (b) && (b) <= 0xFE)
#define IsUDA2(a, b)	(0xF8 <= (a) && (a) <= 0xFE && 0xA1 <= (b) && (b) <= 0xFE)
#define IsUDA3(a, b)	(0xA1 <= (a) && (a) <= 0xA7 && 0x40 <= (b) && (b) <= 0xA0 && (b) != 0x7F)

/*
#define Is1stByte(c)	(((c) - 0x81U) <= (0xFEU - 0x81U))
#define Is2ndByte(c)	(((c) - 0x40U) <= (0xFEU - 0x40U) && (c) != 0x7F)
#define Or2ndByte(c)	(((c) - 0x30U) <= (0x39U - 0x30U))
#define Is3rdByte(c)	(((c) - 0x81U) <= (0xFEU - 0x81U))
#define Is4thByte(c)	(((c) - 0x30U) <= (0x39U - 0x30U))

#define IsUDA1(a, b)	(((a) - 0xAAU) <= (0xAFU - 0xAAU) && ((b) - 0xA1U) <= (0xFEU - 0xA1U))
#define IsUDA2(a, b)	(((a) - 0xF8U) <= (0xFEU - 0xF8U) && ((b) - 0xA1U) <= (0xFEU - 0xA1U))
#define IsUDA3(a, b)	(((a) - 0xA1U) <= (0xA7U - 0xA1U) && ((b) - 0x40U) <= (0xA0U - 0x40U) && (b) != 0x7F)
*/
template <class _T>
void mcGBCodeT<_T>::GB18030ToUnicode(WORD *Dst, int &nDst, BYTE *Src, int nSrc)
{
	BYTE *SrcE = Src + nSrc;
	WORD *DstE = Dst + nDst;
	WORD *DstS = Dst;
	while(Src < SrcE && Dst < DstE)
	{
		if (Src[0] < 0x80)
		{
			*Dst++ = *Src++;
		}
		else if (Is1stByte(Src[0]))
		{
			if (Is2ndByte(Src[1]))
			{
				*Dst++ = GB18030ToUnicode(Src, 2);

				Src += 2;
			}
		//	else if (Or2ndByte(Src[1]) && Is3rdByte(Src[2]) && Is4thByte(Src[3]))
		//	{
		//		*Dst++ = GB18030ToUnicode(Src, 4);
		//		Src += 4;
		//	}
			else
				break;
...
template <class _T>
void mcGBCodeT<_T>::GB18030ToUnicode(WORD *Dst, int &nDst, BYTE *Src)
{
	WORD *DstE = Dst + nDst;
	WORD *DstS = Dst;
	while(Src[0] != 0 && Dst < DstE)
	{
		if (Src[0] < 0x80)
		{
			*Dst++ = *Src++;
...
template <class _T>
void mcGBCodeT<_T>::UnicodeToGB18030(BYTE *Dst, int &nDst, WORD *Src, int nSrc)
{
	WORD *SrcE = Src + nSrc;
	BYTE *DstE = Dst + nDst;
	BYTE *DstS = Dst;
...
template <class _T>
void mcGBCodeT<_T>::UnicodeToGB18030(BYTE *Dst, int &nDst, WORD *Src)
{
	BYTE *DstE = Dst + nDst;
	BYTE *DstS = Dst;
...
template <class _T>
void mcGBCodeT<_T>::UnicodeToUTF8(BYTE *Dst, int &nDst, WORD *Src, int nSrc)
{
	WORD *SrcE = Src + nSrc;
	BYTE *DstE = Dst + nDst;
	BYTE *DstS = Dst;
	...
	while(Src < SrcE && Dst < DstE)
	{
		...
		{
		#ifdef _SUPPORT_32BIT_UNICODE_
		case 6: Dst[5] = (BYTE)(0x80 | (u & 0x3f)); u >>= 6; u |= 0x4000000;
		case 5: Dst[4] = (BYTE)(0x80 | (u & 0x3f)); u >>= 6; u |= 0x200000;
		case 4: Dst[3] = (BYTE)(0x80 | (u & 0x3f)); u >>= 6; u |= 0x10000;
		#endif
		case 3: Dst[2] = (BYTE)(0x80 | (u & 0x3f)); u >>= 6; u |= 0x800;
		case 2: Dst[1] = (BYTE)(0x80 | (u & 0x3f)); u >>= 6; u |= 0xc0;
		case 1: Dst[0] = (BYTE)u;
		}
...
template <class _T>
void mcGBCodeT<_T>::UnicodeToUTF8(BYTE *Dst, int &nDst, WORD *Src)
{
	BYTE *DstE = Dst + nDst;
	BYTE *DstS = Dst;
...
template <class _T>
void mcGBCodeT<_T>::UTF8ToUnicode(WORD *Dst, int &nDst, BYTE *Src, int nSrc)
{
	BYTE *SrcE = Src + nSrc;
	WORD *DstE = Dst + nDst;
	WORD *DstS = Dst;
	...
	//	}
		...
		{
			*Dst++ = ((WORD)(Src[0] & 0x1f) << 6)  |
					 ((WORD)(Src[1] ^ 0x80));
			Src += 2;
		}
		else if (Src[0] < 0xf0)
		{
			*Dst++ = ((WORD)(Src[0] & 0x0f) << 12) |
					 ((WORD)(Src[1] ^ 0x80) << 6)  |
					 ((WORD)(Src[2] ^ 0x80));
...
template <class _T>
void mcGBCodeT<_T>::UTF8ToUnicode(WORD *Dst, int &nDst, BYTE *Src)
{
	WORD *DstE = Dst + nDst;
	WORD *DstS = Dst;
	while(Src[0] !=0 && Dst < DstE)
...
template <class _T>
WORD mcGBCodeT<_T>::GB18030ToUnicode(BYTE *Src, int nLen)
{
	const static WORD CTab[22046] =
	{
		/* GB 0x8140..0x817E */
		...
		0xE864,
		/* Skip: GB 0xFEA1..0xFEFE (UDA 2) */
	};

	const static WORD FTab[6793] =
	{
	...
	case 2:
		{
			...
			WORD Unicode;
			if (IsUDA1(c1, c2))
				Unicode = 0xE000 + (c1 - 0xAA) * 94 + (c2 - 0xA1);
			else if (IsUDA2(c1, c2))
				Unicode = 0xE234 + (c1 - 0xF8) * 94 + (c2 - 0xA1);
			else if (IsUDA3(c1, c2))
				Unicode = 0xE4C6 + (c1 - 0xA1) * 96 + (c2 - 0x40) - ((c2 >= 0x80) ? 1 : 0);
			else
			{
				// Use the mapping table
				UINT i = (c1 - 0x81) * 190 + (c2 - 0x40) - ((c2 >= 0x80) ? 1 : 0);
	...
	case 4:
		{
			...
			UINT n = (c1 - 0x81) * 12600 + (c2 - 0x30) * 1260 + (c3 - 0x81) * 10 + (c4 - 0x30);
			if (n <= 0x99FB)
			{
				/* GB+81308130 - GB+8431A439 */
				const IdxTab *IdxG2U = &ITab[n >> 8];

				if ((n & 0xFF) >= IdxG2U->Head && (n & 0xFF) <= IdxG2U->Tail)
				{
					Unicode = FTab[n - IdxG2U->Tbls];
				}
				else
				{
					Unicode = IdxG2U->Algs + (n & 0xFF);
				}
			}
			else if (0x2E248 <= n && n <= 0x12E247)
			{
...
template <class _T>
void mcGBCodeT<_T>::UnicodeToGB18030(BYTE *Dst, WORD u)
{
	const static struct IdxTab
	{
		WORD Head;
	...
	WORD g = 0;
	if (u <= 0xD7FF || 0xE766 <= u)
	{
		const IdxTab *IdxU2G = &ITab[u >> 8];

		if ((u & 0xFF) >= IdxU2G->Head && (u & 0xFF) <= IdxU2G->Tail)
		{
			g = CTab[u - IdxU2G->Tbls];
		//	if (g > 0x8000)
		//		success.
		}
	}
	else if (0xE000 <= u && u <= 0xE765)
	{
		if (u <= 0xE233)
			g = 0xAAA1 + (((u - 0xE000) / 94) << 8) + (u - 0xE000) % 94;
		else if (u <= 0xE4C5)
			g = 0xF8A1 + (((u - 0xE234) / 94) << 8) + (u - 0xE234) % 94;
		else
		{
			g = 0xA140 + (((u - 0xE4C6) / 96) << 8) + (u - 0xE4C6) % 96;
			if ((g & 0xFF) >= 0x7F)
				g++;
		}
	}
...
#endif   //__MAPTRIXC_GB18030_LIB__
